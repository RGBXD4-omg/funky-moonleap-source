...
class Main extends Sprite
{
	...
	public static var gameVersion:String = '2.1';
	public static var gFont:String = Paths.font('pixel-berry.ttf');
	...
	**/
	...
	public static function switchState(target:FlxState)
	{
		...
		// just switches without transition
		FlxG.switchState(target);
	}
	...
	public static function checkAntialiasing(Object:FlxBasic):Void
	{
		if(SaveData.trueSettings.get('Antialiasing'))
			return;
		
		// turns off flxsprites antialiasing
		if(Std.isOfType(Object, FlxSprite))
			cast(Object, FlxSprite).antialiasing = false;
		
		// loops inside every group inside the main groups
		// so it can turn off the strumlines/notes antialiasing
		if(Object is FlxTypedGroup)
		{
			function checkGroup(grp:FlxBasic)
			{
				for(i in cast(grp, FlxTypedGroup<Dynamic>).members)
				{
					if(Std.isOfType(i, FlxSprite))
						cast(i, FlxSprite).antialiasing = false;
					if(i is FlxTypedGroup) // if theres a group inside the group, it loops this code
						checkGroup(i);
				}
			}
			// checks the main object first
			checkGroup(Object);
		}
	}
}
